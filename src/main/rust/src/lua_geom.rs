...
macro_rules! assert_stopfns_type {
    ($L:expr) => ({
        lua_rawgeti($L, LUA_REGISTRYINDEX, GLDRAW_LUA_STOPFNS); {
...
unsafe fn create_sandbox(L: *mut lua_State) {
    lua_rawgeti(L, LUA_REGISTRYINDEX, GLDRAW_LUA_CREATE_SANDBOX);
    lua_pcall(L, 0, 1, 0);
}
